...
reponame="umami"
umami_version="postgresql-v2.19.0"

# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-umami container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-umami; then
	echo "Pulling NodeJS runtime..."
	buildah from --name nodebuilder-umami -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
	...
	--label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 ghcr.io/umami-software/umami:$umami_version" \
	"${container}"
# Commit the image
